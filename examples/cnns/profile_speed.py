...
FilePath: /Unknow/profile_speed.py
'''
...
from optimizers import AdamW, Adam, SGD, Prodigy, SGD_boost
...
def init_optimizer(model_name='resnet50', opt='SupEffAdaSNR', model=None):
    ...
    if opt == "SGD_boost":
        optimizer = SGD_boost(optim_groups, lr=lr, momentum=0.9, eps=1e-08, weight_decay=weight_decay)
    elif opt == "Adam":
...
def torch_profiler_gpt2(num_iters=5, optimizer=None, inputs=None, labels=None, model=None, args=None):
    ...
    global OPTIMIZER
    need_warmup = (OPTIMIZER == "SGD_boost")
