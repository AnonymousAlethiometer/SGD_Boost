...
'''
Author: Unknow
Date: 2024-02-26 14:21:03
LastEditTime: 2024-03-05 14:44:48
LastEditors: Unknow
Description: 
FilePath: /Unknow/utils/misc.py
'''
...
def readable_nparams(model):
    return f'nparams: {nparams(model):,}'
...
class AverageMeterManager():
    def __init__(self) -> None:
        self.metrics = {'train': {}, 'test': {}}  # metrics for train and test

    def update(self, key, value, is_train=True, n=1):
        if is_train:
            prefix = 'train'
        else:
            prefix = 'test'
        ...
        if k not in self.metrics[prefix]:
            self.metrics[prefix][k] = AverageMeter()
        self.metrics[prefix][k].update(val=value, n=n)
    ...
    def reset(self, is_train=True):
        if is_train:
            ...
            prefix = 'test'
        for k, v in self.metrics[prefix].items():
            v.reset()

    def compute(self, key, is_train=True):
        if is_train:
            prefix = 'train'
        else:
            prefix = 'test'
        k = f'{prefix}_{key}'
        if k not in self.metrics[prefix]:
            raise ValueError(f'Cannot compute metric: {k} before update.')
        return self.metrics[prefix][k].compute()
