...
FilePath: /Unknow/utils/optimizer.py
'''
import os
...
from optimizers import Prodigy, SGD_boost
...
def optimizers(net, opti_name, lr, weight_decay, **kwargs):
    ...
    elif opti_name == 'SGD-boost-profiling':
        return SGD_boost(net.parameters(), lr=lr, momentum=0.9, weight_decay=weight_decay, nesterov=False)
    # ------- profiling the optimizers -------
    else:
...
def record_optimizer_info(optimizer, writer: SummaryWriter = None, current_step: int=0, current_epoch: int=0, extra_save_pth: str=None):
    ...
    grad_snrs = []
    gard_norms = []
    if type(optimizer) in [SGD_boost]:
